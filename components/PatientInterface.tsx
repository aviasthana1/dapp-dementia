          ...
          {reminders.length === 0 ? (
            ...
            reminders.map((r) => (
              <li key={r.id}>
                <button
                  type="button"
                  onClick={() => setAlertReminder(r)}
                  className="card p-5 rounded-2xl flex items-start gap-4 border-l-4 border-blue-500 w-full text-left hover:bg-blue-50 transition-colors"
                >
                  {r.photoUrl ? (
                    <img
                      src={r.photoUrl}
